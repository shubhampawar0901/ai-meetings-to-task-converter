...
async function intelligentMeetingTranscriptAnalysis(inputText) {
  try {
    // Use OpenAI for intelligent meeting transcript analysis
    const results = await openaiService.parseTasksFromMeetingTranscript(inputText);
    console.log('AI-powered meeting transcript analysis results:', results);
    return results;
  } catch (error) {
...
async function intelligentTextAnalysis(inputText) {
  const results = await intelligentMeetingTranscriptAnalysis(inputText);
...
class TaskItemService {
  ...
  async processAndCreateTaskItems(inputText) {
    try {
      // Use AI-powered intelligent meeting transcript analysis
      let taskItems = await intelligentMeetingTranscriptAnalysis(inputText);
  ...
  // Keep the old single task method for backward compatibility
  async processAndCreateSingleTaskItem(inputText) {
    try {
      // Use AI-powered intelligent text analysis for single task
      ...
      console.error('Error in processAndCreateSingleTaskItem:', error);
      throw error;
    }
  }
