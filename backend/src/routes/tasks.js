...
// Process meeting transcript and create multiple task items
router.post('/process-request', async (req, res) => {
    try {
        ...
        // Use enhanced task item service for processing meeting transcripts
        const taskItems = await taskItemService.processAndCreateTaskItems(text);
        res.status(201).json({ taskItems });
    } catch (error) {
        console.error('Error processing request:', error);
        res.status(500).json({ error: 'Failed to process request and create task items' });
    }
});
